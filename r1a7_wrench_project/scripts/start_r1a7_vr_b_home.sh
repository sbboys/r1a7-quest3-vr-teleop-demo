...
AUTO_HOME_SCRIPT="${PROJECT_ROOT}/r1a7_wrench_project/scripts/auto_move_to_keyframe_v2_1.py"
...
check_conflicts() {
    local found
    found="$(pgrep -af 'python.*(r1a7_vr_dual_arm_g1ik_real.py|auto_move_to_keyframe_v2_1.py)' || true)"
    if [ -n "${found}" ]; then
...
run_vr() {
    ...
    ' bash "${pgid_file}" \
        ...
        --record_episode_id wrench_grasp_001 \
