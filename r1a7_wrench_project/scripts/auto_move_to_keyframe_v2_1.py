...
class AutoKeyframeMoverV21:
    ...
    def __init__(
        ...
        self.target_gripper_q: Optional[
            np.ndarray
        ] = None
    ...
    def execute(
        ...
        if (
            self.args.use_keyframe_gripper
            and
            self.target_gripper_q is not None
            and
            self.gripper_hold_q is not None
        ):

            target = (
                self.gripper_hold_q.copy()
            )

            for i in range(2):
                if np.isfinite(
                    self.target_gripper_q[i]
                ):
                    target[i] = float(
                        self.target_gripper_q[i]
                    )

            self.gripper_hold_q = target

            print(
                "Keyframe gripper target : "
                f"left={self.gripper_hold_q[0]:+.6f}, "
                f"right={self.gripper_hold_q[1]:+.6f}"
            )
    ...
    def apply_target_gripper(
        self,
        target_gripper_q: Optional[np.ndarray],
    ) -> None:

        if (
            not self.args.use_keyframe_gripper
            or
            target_gripper_q is None
            or
            self.gripper_hold_q is None
        ):
    ...
    def execute_sequence(
        ...
        grippers: list[Optional[np.ndarray]],
    ) -> int:

        if len(names) != len(targets) or len(names) != len(grippers):
            raise RuntimeError(
                "sequence names, targets, and grippers must have the same length"
            )
        ...
        print(
            "REAL ROBOT KEYFRAME SEQUENCE V2.1 ARMED"
        )
        ...
        print(
            f"Initial first-stage error: "
            f"{global_error:.6f} rad"
        )

        print(
            "LowCmd will remain active across all stages "
            "and release only after the final stage or abort."
        )
        ...
        try:
            ...
            for index, (
                ...
                print(
                    f"SEQUENCE STAGE {index}/{len(names)}: {name}"
                )
                ...
                if not ok:
                    ...
                    print(
                        f"SEQUENCE ABORTED: {name} failed."
                    )
...
def build_parser():
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Comma-separated keyframes to execute in one continuous LowCmd "
            "session. The positional keyframe is ignored when this is set."
        ),
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Skip adaptive wrist/arm recovery and execute one whole-arm "
            "joint-space trajectory. Use for demonstration replay keyframes, "
            "not for unknown HOME recovery."
        ),
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the interactive confirmation. Intended only for wrapper "
            "scripts that already performed their own safety confirmation."
        ),
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Command finite left/right gripper positions from the keyframe. "
            "Unset by default to preserve existing HOME behavior."
        ),
    )
...
def main():
    ...
    sequence_names = [
        name.strip().upper()
        for name in args.sequence.split(",")
        if name.strip()
    ]
